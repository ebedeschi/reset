 ...
 *  @arg    '2' if no answer 
 */
uint8_t arduinoLoRaWAN::ON(uint8_t uart)
{
    uint8_t error;
    _baudrate = 57600;
    _uart = uart;
...
{   
    ...
    sprintf(ans2,(char*)pgm_read_word(&(table_LoRaWAN_ANSWERS[20])));
    // create "invalid_param" answer
    sprintf(ans3,(char*)pgm_read_word(&(table_LoRaWAN_ANSWERS[21])));
...
uint8_t arduinoLoRaWAN::getDeviceEUI()
{
    ...
    // create "invalid_param" answer
    sprintf(ans1,(char*)pgm_read_word(&(table_LoRaWAN_ANSWERS[21])));
...
uint8_t arduinoLoRaWAN::setRetries(uint8_t retries)
{
    ...
    sprintf(_command,(char*)pgm_read_word(&(table_LoRaWAN_COMMANDS[69])),(uint8_t)retries);
    // create "ok" answer
    sprintf(ans1,(char*)pgm_read_word(&(table_LoRaWAN_ANSWERS[0])));
    // create "invalid_param" answer
    sprintf(ans2,(char*)pgm_read_word(&(table_LoRaWAN_ANSWERS[1])));
...
uint8_t arduinoLoRaWAN::setRadioSF(char* sprfact)
{
    ...
    // create "ok" answer
    sprintf(ans1,(char*)pgm_read_word(&(table_LoRaWAN_ANSWERS[22])));
    // create "invalid_param" answer
    sprintf(ans2,(char*)pgm_read_word(&(table_LoRaWAN_ANSWERS[21])));
...
/*!
 * @brief   This function puts the system to Sleep for the specified number of milliseconds
 *
 * @param   uint64_t length: decimal number representing the number of milliseconds
 * 			the system is put to Sleep, from 100 to 4294967296
 *
 * @remarks The module can be forced to exit from Sleep
 * 			by sending a break condition followed by a
 * 			0x55 character at the new baud rate. Note that
 * 			the break condition needs to be long enough not
 * 			to be interpreted as a valid character at the current baud rate
 *
 * @return
 *  @arg    '0' if OK
 *  @arg    '1' if error
 *  @arg    '2' if no answer
 */
uint8_t arduinoLoRaWAN::sleep(uint64_t length)
{
    uint8_t status;
    char ans1[15];
    char ans2[15];
    ...
    // create "sys sleep length" command
    sprintf(_command,(char*)pgm_read_word(&(table_LoRaWAN_COMMANDS[81])),length);
    // create "ok" answer
    sprintf(ans1,(char*)pgm_read_word(&(table_LoRaWAN_ANSWERS[0])));
    // create "invalid_param" answer
    sprintf(ans2,(char*)pgm_read_word(&(table_LoRaWAN_ANSWERS[1])));
...
uint8_t arduinoLoRaWAN::wakeUP()
{
    ...
    // excape command 0x00 0x55
    _command[0]=(uint8_t)0x00;
    _command[1]=(uint8_t)0x55;
    // create "ok" answer
    sprintf(ans1,(char*)pgm_read_word(&(table_LoRaWAN_ANSWERS[0])));

    sendCommand((uint8_t*)_command, 2);

    status = waitFor(ans1, 100);
